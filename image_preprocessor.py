...
'''
This function applies a mask upon data. The result of this operation is returned. The only condition is that
the mask and the data list must be the same size. If this is not the situation, this function will raise an error.
Parameters:
...
def apply_mask(data: list, mask: list) -> int:
        ...
        for y in range(edge_buffer_size, img_height-edge_buffer_size):
            ...
            for get_old_x_index in range(len(mask_horizontal_edges)):
                ...
                for get_old_y_index in range(len(mask_horizontal_edges)):
                    old_values[get_old_x_index].append(original.getpixel((x - edge_buffer_size + get_old_x_index,
                                                                          y - edge_buffer_size + get_old_y_index)))

            # apply x filter
            result_horizontal_mask = apply_mask(old_values, mask_horizontal_edges)
            # apply y filter
            result_vertical_mask = apply_mask(old_values, mask_vertical_edges)

            # average filter results
            # https://en.wikipedia.org/wiki/Sobel_operator <-- this explains the square root
            # on the sum of the squares. They do it here in the pseudocode, that might give better results
            average_result = int(math.sqrt(result_horizontal_mask*result_horizontal_mask +
                                           result_vertical_mask*result_vertical_mask))
